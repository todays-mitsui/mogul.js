...
const { Parser } = require('../src/js/Parser/UnlambdaStyleParser')
...
describe('UnlambdaStyleParser', function () {
  describe('変数のパーズ', function() {
    it('1文字変数のパーズ', function () {
      const src = 'x';
      const parseResult = Parser.variable.parse(src);
  ...
  describe('シンボルのパーズ', function() {
    it(':から始まるトークンはシンボルを作る', function() {
      const src = ':x';
      const parseResult = Parser.symbl.parse(src);
  ...
  describe('適用式のパーズ', function() {
    it('単純な適用', function () {
      const src = '`xy';
      const parseResult = Parser.apply.parse(src);
  ...
  describe('関数定義', function() {
    it('関数定義の左辺値', function() {
      const src = '```fxyz';
      const parseResult = Parser.lvalue.parse(src);
    ...
    it('関数定義', function() {
      const src = '``fxy=`yx';
      const parseResult = Parser.def.parse(src);
